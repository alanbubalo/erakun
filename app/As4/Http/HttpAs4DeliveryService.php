...
final readonly class HttpAs4DeliveryService implements As4DeliveryService
{
    ...
    public function send(string $ublXml, string $senderOib, string $recipientOib): As4DeliveryReceipt
    {
        ...
        try {
            ...
            throw new As4DeliveryException('TRANSPORT', $messageId, 'AS4 peer unreachable: '.$e->getMessage());
        }

        return $this->parseResponse($response, $messageId);
    }
    ...
    private function parseResponse(Response $response, string $messageId): As4DeliveryReceipt
    {
        $raw = $response->body();

        throw_if($raw === '', new As4DeliveryException('TRANSPORT', $messageId, "AS4 peer returned an empty body (HTTP {$response->status()})."));
        ...
        throw_if($loaded === false, new As4DeliveryException('TRANSPORT', $messageId, "AS4 peer returned malformed XML (HTTP {$response->status()})."));
        ...
        throw_unless($signal instanceof DOMElement, new As4DeliveryException('TRANSPORT', $messageId, 'AS4 peer response is missing eb:SignalMessage.'));

        $error = $xpath->query('eb:Error', $signal)->item(0);

        if ($error instanceof DOMElement) {
            throw $this->buildErrorException($error, $messageId);
        }

        $receipt = $xpath->query('eb:Receipt', $signal)->item(0);

        throw_unless($receipt instanceof DOMElement, new As4DeliveryException('TRANSPORT', $messageId, 'AS4 peer signal is neither eb:Receipt nor eb:Error.'));

        return $this->buildReceipt($xpath, $signal, $messageId, $raw);
    }

    private function buildErrorException(DOMElement $error, string $messageId): As4DeliveryException
    {
        ...
        return new As4DeliveryException($errorCode, $messageId, $message);
    }
    ...
    private function buildReceipt(
        ...
        try {
            $receiptMessageId = $this->requireText($xpath, $signal, 'eb:MessageInfo/eb:MessageId', $messageId);
            $timestamp = $this->requireText($xpath, $signal, 'eb:MessageInfo/eb:Timestamp', $messageId);
            $acknowledgedAt = CarbonImmutable::parse($timestamp);
        } catch (RuntimeException $e) {
            throw new As4DeliveryException('TRANSPORT', $messageId, 'AS4 peer returned an unparseable receipt: '.$e->getMessage());
        }
    ...
    private function requireText(DOMXPath $xpath, DOMElement $context, string $expression, string $messageId): string
    {
        $node = $xpath->query($expression, $context)->item(0);
        $value = $node === null ? '' : trim((string) $node->textContent);

        throw_if($value === '', new As4DeliveryException('TRANSPORT', $messageId, "Missing required field in AS4 receipt: {$expression}"));
